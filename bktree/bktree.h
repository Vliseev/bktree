...
class bktree_node {
    ...
    OutputIt find(const T &value, const Unit &limit, const Metric &distance,
                  OutputIt it) const {
        Unit dist = distance(value, this->value_);
        if (dist <= limit) {
            // Found one
...
class bktree {
   private:
    ...
    typedef typename detail::bktree_node<T, Unit, Metric>::node_ptr_type
        node_ptr_type;
   ...
   public:
    bktree(const Metric &distance = Metric())
        ...
        static_assert(std::is_integral<Unit>::value,
                      "Integral unit type required.");
    }
