...
#ifndef BKTREE_LEV_WSTRING_H
#define BKTREE_LEV_WSTRING_H
#include <algorithm>
#include <string>
#include <vector>
...
class levenshtein_distance {
   ...
   public:
    ...
    size_t operator()(const std::wstring &s1, const std::wstring &s2) const {
        ...
        if (costs_.size() < std::max(n, m)) costs_.resize(std::max(n, m));
